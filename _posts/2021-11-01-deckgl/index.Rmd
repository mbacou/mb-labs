...
if(interactive()) setwd("_posts/2021-11-06-deckgl/")
...
adm[, "color"] <- colour_ramp(unname(pal[1:3]))(seq(0, 1, length=nrow(adm)))
...
Load terrain and satellite basemaps. Then we progressively add water features.
...
props_basemap = list(
  ...
  elevationData = sprintf(mapbox_api, terrain_img, token),
  texture = sprintf(mapbox_api, surface_img, token),
  wireframe = FALSE,
  color = c(255, 255, 255)
)
...
Using Mapbox experimental vector tile basemap below.
