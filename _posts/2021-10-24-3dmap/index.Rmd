...
base_map %>% plot_3d(resize_matrix(srtm_array, 1/10),
  zscale=20, zoom=0.5,
...
base_map <- srtm_array %>% 
  height_shade(terrain.colors(256)) %>% 
  add_overlay(sphere_shade(srtm_array, texture="desert"), alphalayer=0.4) %>%
...
2D view of the generated basemap.
...
place_layer <- generate_label_overlay(places,
  ...
  halo_color=pal["black"], halo_expand=3, halo_blur=3, halo_alpha=.7, seed=1,
...
```{r, rgl.margin=0, fig.cap="Step 2: Interactive 3D Scene of Niger River Basin"}

scene <- readRDS("./_data/scene_basin.rds")
scene %>% plot_3d(resize_matrix(srtm_array, 1/5), zscale=40, 
  linewidth=0, theta=30, phi=45, fov=0, zoom=0.5, 
  family="Roboto Condensed",
  shadow=TRUE, shadowcolor=pal["black"], solidcolor=pal["black"])
...
Similar implementation using **Three.js** libraries instead of WebGL. The scene is **12 MB in size** (using a low resampling rate), it was generated using the [Qgis2threejs](https://qgis2threejs.readthedocs.io/en/docs/) plugin. The main difference is we're showing **ESA CCI land cover classification** as an image overlay instead of a texture.
