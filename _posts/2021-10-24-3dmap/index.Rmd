...
This notebook is **Part 3** of an exploration to visualize results of hydrologic models. In [Part 1](../2021-10-16-svg/) we built custom HTML widgets using `D3.js`, and in [Part 2](../2021-10-12-highcharts/) we looked at rendering water fluxes using Sankey diagrams. Here we test multiple libraries to generate hillshade (3D) views of river basins and water infrastructure, in particular we want to compare [leaflet](https://leafletjs.com/), [Three.js](https://threejs.org/) and [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API) implementations.
...
Next we convert the 2 rasters to a matrix format that's compatible with [Rayshader](https://github.com/tylermorganwall/rayshader/) hillshading methods.
...
That doesn't look very clear, so instead we'll create a basemap, not using the satellite image but a built-in desert texture.
...
Looks better, so let's acquire and overlay spatial features from OSM. Note that water-related features for Mali are scarcely available from OSM, but network streams are recorded.
...
Following the approach above, we experiment with a 3D scene for the entire Niger River basin area used in the analysis.
...
Finally we'll use WebGL to render this scene in 3D (place names need additional work).

```{r, fig.cap="Step 2: Interactive 3D Scene of Niger River Basin"}

scene <- readRDS("./_data/scene_basin.rds")

scene %>% plot_3d(resize_matrix(srtm_array, 1/5), zscale=80, 
...
Similar implementation using **Three.js** libraries and viewer instead of WebGL. The scene is **12 MB in size** (using a low resampling rate), it was generated using the [Qgis2threejs](https://qgis2threejs.readthedocs.io/en/docs/) plugin. The main difference is we're showing **ESA CCI land cover classification** as an image overlay instead of a texture.

```{r, fig.cap="3D Scene of Niger River Basin with Land Cover Overlay"}

knitr::include_graphics("./fig/threejs_luc.png")

```

This [widget is rendered](./threejs_files/) here. 
