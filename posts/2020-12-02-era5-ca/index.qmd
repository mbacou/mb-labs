...
description : ERA5-LAND temperature estimate biases against ground observations collected from NOAA GHCN network of meteorological stations.
date: 2020-12-02
draft: false
freeze: true
fig-format: png
...
load(here("./_data/2020-burn_olam_usa_07.RData"))
```
...
rm(x, i, j, t, tmp)
save.image(here("./_data/2020-burn_olam_usa_07.RData"))
```
...
plot(
  r[[1]],
  ext = ext(stations.sp[stations.sp$id %in% ids, ]) + c(1, 1, 0, 0),
...
gglabs(tmp[year(date) %between% c(2010, 2014)], aes(sdate, color = sign)) +
  geom_segment(
...
gglabs(tmp[year(date) > 2014], aes(sdate, color = sign)) +
  geom_segment(
    aes(xend = sdate, y = Tmin, yend = GHCN_Tmin),
    size = .8,
    lineend = "butt"
