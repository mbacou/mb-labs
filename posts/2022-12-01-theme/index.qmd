...
::: {.column-page-inset-right .d-flex .ink .ink-box .p-2}
No other reason for this map below but to showcase Quarto's new content layout features -- and well, I love Soviet-era classic topo styles. Check out Russian [cartography](https://sovietmaps.com/), it is stunning work. 

This margin note uses a custom `<div>` with CSS classes `.column-page-inset-right .d-flex` making use of Boostrap's and Quarto utilities.
:::
...
::: {.column-body .shaded .mb-2}
`.column-body .striped`
:::

::: {.column-body-outset .shaded .mb-2}
...
::: {.column-page-inset .shaded .mb-2}
`.column-page-inset .striped`
:::

::: {.column-screen .shaded .mb-2}
...
[^4]: KDE, 30 years later, still going strong! This same color scheme selected in VSCode and Positron.
...
Moreover as of `Quarto v1.6`, the engine now allows for standard branding elements (colors, fonts) to be defined in standalone `_brand.yml` files on a per-document or per-project basis[^brand]. 
...
Bootstrap default line-height and font sizing typically work, but I like to enforce a strict vertical baseline whenever possible[^vr], so I make sure to define all vertical spacing and padding as multiple of a standard line height of `$spacer`.

[^vr]: About **vertical rhythm** amd why it matters for content readability, see [Klaas Leussink](https://hnldesign.hashnode.dev/setting-a-flexible-baseline-grid-in-css) article on his personal blog.
...
No additional changes to font definitions aside from using `Crimson Pro` and `M+ Code` fonts[^mplus]. Here are **bold**, *italic*, and <small>smaller</small> text spans. Here is ^superscript^ and ~subscript~ text, and ~~strikethrough~~ notes. Semantic colors with e.g. [.text-info]{.text-info} provide accented text. I have added 2 background classes for my own use, a semi-transparent and blurred [`.shaded`]{.shaded} and a [`.striped`]{.striped}. This theme uses [.text-muted]{.text-muted} for all post metadata, for end- and margin-notes, and for captioning.

[^mplus]: M+ font system was designed by Japanese artist [Coji Morishita](https://mplusfonts.github.io/) with extended support for Latin, Japanese, Cyrillic.  It is an excellent monospace font for smaller display sizes.
