---
title: "Rainfall: Merge Ground and Satellite Observations with RFMerge"
...
While estimating winter season frost risk for almond orchards over Central California, project partners reported significant discrepancies between modeled temperature estimates provided by **ERA5-LAND Reanalysis** dataset and ground observations. These biases impact the calculation of the daily **Frost-Degree-Day** index (FDD) used to price insurance coverage.

Here we want to recalibrate [ERA5-LAND](https://cds.climate.copernicus.eu/cdsapp#!/dataset/reanalysis-era5-land) hourly temperatures to [CIMIS](https://cimis.water.ca.gov/) ground network observations in order to address ERA5 model biases reported over Central California. The entire procedure involves the following steps:
...
This work was initially performed for **WorldCover PBC** and **OLAM US** in early 2021. It is somewhat redacted here to omit confidential details.
...
if (interactive()) {
  setwd("./posts/2021-01-16-rfmerge/")
}

load("_data/2020-burn_olam_usa_06.RData")

pal <- pal(file = "../../_brand.yml")
my.settings <- list(
...
Define zone of interest (ZOI), load and verify covariate layers.

```{r zoi}
#| eval: false

# Clear workspace, keep only portfolio details
load("./tmp/2020-burn_olam_usa_04.RData")
...
# Limit ZOI to counties of interest to OLAM
...
f <- list.files(
  "../maps/ERA5LAND/2m_temperature/hourly/usa.ca",
  ".nc",
  recursive = T,
  full.names = T
)
...
Next we obtain long-term weather station records from the CIMIS API.
...
obs.gis <- SpatialPointsDataFrame(
  obs.gis[, .(X, Y)],
  data.frame(obs.gis),
  proj4string = CRS("+init=epsg:4326")
)
...
plot(
  obs.gis,
  add = T,
  col = pal[c("red", "green")][1 + (obs.gis$IsActive == "True")],
  pch = "x"
)
...
rfmep <- RFmerge(
  ...
  training = 0.9,
  write2disk = TRUE,
  drty.out = "./out/2017_RFmerge_proj",
  parallel = "parallel",
  par.nnodes = mc.cores
)
...
dt.imp[,
  time := as.POSIXct(time, format = "X%Y.%m.%d.%H.%M.%OS", tz = "America/Los_Angeles")
]
...
dt <- readRDS("./tmp/2020-tmean-hourly_olam_ca_almd.rds")
...
saveRDS(dt, "./tmp/2020-tmean-hourly_olam_ca_almd_imputed.rds")
```
...
save.image("./tmp/2020-burn_olam_usa_06.RData")
```
...
library(terra)
era <- rast("./_out/ERA5_2m_temperature_2017.grd")
...
plot(
  ...
  sub = "ERA5-LAND Temperature (°C) on 2017-01-01 01:00"
)
grid()
...
plot(
  stations,
  add = T,
  col = pal[c("dark", "red")][1 + (obs.gis$IsActive == "True")],
  pch = "x"
)
legend(
  "bottom",
...
plot(
  obs.ts[, 1],
  col = alpha(pal[1], .7),
  sub = paste("Station", names(obs.ts)[1]),
  xlab = "CIMIS Hourly Temp. °C",
  ylab = NA
)
grid()
...
plot(
  obs.ts[, 20],
  col = alpha(pal[1], .7),
  sub = paste("Station", names(obs.ts)[20]),
  xlab = "CIMIS Hourly Temp. °C",
  ylab = NA
)
grid()
...
plot(
  ...
  breaks = -8:11,
  sub = "RF-MEP Temperatures °C"
)
grid()
plot(pts, pch = "o", col = pal["dark"], add = T)
plot(
  stations[stations$StationNbr %in% names(obs.ts), ],
  pch = "x",
  col = pal[["dark"]],
  add = T
)
legend(
  ...
  ncol = 2,
  bg = pal["light"],
  legend = c("Active station", "Production site"),
  pch = c("x", "o")
)
...
wireframe(
  Value ~ as.numeric(Date) * Time,
  data = obs,
  xlab = "Date",
  ylab = "Hour",
  sub = "Nearest CIMIS Stations - Hourly Temp. °C",
  col = NA,
  col.regions = terrain.colors(100, alpha = .4),
  drape = T,
  at = do.breaks(c(-5, 35), 100)
)

wireframe(
  era ~ as.numeric(date) * time,
  data = dt.imp,
  xlab = "Date",
  ylab = "Hour",
  zlab = "°C",
...
wireframe(
  (value - era) ~ as.numeric(date) * time,
  data = dt.imp,
  xlab = "Date",
  ylab = "Hour",
  zlab = "°C",
...
dt.imp[
  date %between% c("2017-02-01", "2017-03-31"),
  .(
    era = min(era, na.rm = T),
  ...
  .(
    ...
    `FDD (validate)` = sum(
      fifelse(era_valid < 0, pmax(era_valid, -4, na.rm = T), 0),
      na.rm = T
    ),
