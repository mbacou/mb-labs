...
# IMWI color palette
tmp <- fread(here("data/IWMI.gpl"), skip = 4, header = F)
pal <- grDevices::rgb(tmp[, .(V1, V2, V3)], maxColorValue = 255)
names(pal) <- tmp[, V4]
# Save for reuse
saveRDS(pal, here("data/pal_iwmi.rds"))
...
cat(
  ...
  var pal = %s;
    ...
    toJSON(data[iso3 == "ken" & year == 2017]),
    toJSON(pal)
  )
)
```
...
list.files(here("./svg"), pattern = "*.svg", full.names = TRUE)[
  -c(1:3, 7, 9)
] |>
...
```{js}
//| eval: false
//| code: xfun::read_utf8("./js/sheet_1.js")
