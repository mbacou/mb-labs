...
```{=html}
  ...
  <% for (const item of items) { %>
    <div class="grid gap-md-3 py-3" <%= metadataAttrs(item) %> >
      ...
      <div class="g-col-12 g-col-md-3 order-md-1" style="overflow: clip; max-height: <%- templateParams['image-height'] %>;">
        <a href="<%- item.path %>" class="no-external">
        <% const imgSrc = getImage(item) || (templateParams['image-placeholder']); %>
        <% if (imgSrc) { %>
        <img src="<%- imgSrc %>" alt="<%- item['image-alt'] || item.title %>" class="img-fluid rounded shadow-md img-zoom mt-2" />
        <% } %>
        </a>
      </div>
